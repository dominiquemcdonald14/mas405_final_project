...
for(i in 1:length(artist$Artist)){
  # #The elements in each list will be indexed by artist name. but I don't have the names yet.
  # artist_name <- artist$Artist[i]
  # names(artist_keyword)[i] <-  
  # 
  kw <- rep(NA, 10)
  frq <- rep(NA, 10)
  prp <- rep(NA, 10)
  ...
  art_doc <- tm_map(art_doc, removeWords, c("aint", "ooh", "thou", "never", "yeah", "hey", "though", "just", "will", "dont", "gonna", "can", "like", "much", "let",
                                  "upon", "from", "nah", "shall", "now")) 
  # Remove punctuations
