...
artist_keyword <- data.frame(artist = artists_complete$artist_names, sent = rep(NA, 49), word = rep(NA,49), freq_df = rep(NA,49), prop_df = rep(NA,49), keyword = rep(NA,49))


artist_text <- Corpus(VectorSource(artists_complete[,4])); artist_text
...
for(i in 1:length(artists_complete[,2])){
 ...
 # artist_keyword[[i]]$proportion[1:10] <- artist_dtm_d$freq[1:10]/art_num_terms #populating proportions
  ...
  ## Sentiment Analysis
  art_data <- gsub("[,.;:?!]", "", artists_complete[i,4])
  ...
  emotions_prop <- emotions / sum(emotions)
...
```{r}
artist_keyword[order(artist_keyword$word, decreasing = TRUE),] 
artist_keyword[order(artist_keyword$sent),] 
```
